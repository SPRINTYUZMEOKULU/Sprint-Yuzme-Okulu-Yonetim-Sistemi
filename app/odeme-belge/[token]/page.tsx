...
export default async function PaymentDocumentPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="doc-page"><section className="doc-card">
    ...
    {expired ? <div className="notice error">Bu güvenli bağlantının süresi dolmuş. Yeni ödeme bağlantısı için Sprint Yüzme Okulu ile iletişime geçiniz.</div> : completed ? <div className="notice success"><b>Belge bilgileriniz kaydedildi.</b><br/>Ödeme bilgileriniz bu kayıtla eşleştirilecektir. Belgeniz ödeme onayından sonra hazırlanacaktır.</div> : <><h1>Belge Bilgileriniz</h1><p className="intro">IBAN/Havale/EFT ödemenizin belgesini doğru bilgilerle düzenleyebilmemiz için bilgileri ödeme yapacak kişi olarak siz giriniz.</p><CustomerDocumentForm token={token} /></>}
