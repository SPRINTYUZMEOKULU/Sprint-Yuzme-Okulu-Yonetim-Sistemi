...
async function syncPaymentDueNotifications(supabase: SupabaseClient, organizationId: string) {
  ...
  const [studentsResult,packagesResult,paymentsResult,existingResult] = await Promise.all([
    ...
    // Okunmuş bildirimleri de getiriyoruz. Aynı kayıt + aynı vade için bildirim
    // kullanıcı kapattıktan sonra tekrar oluşturulmamalı.
  ...
  // Aynı enrollment için vade tarihi değişirse yeni bir bildirim hakkı doğar;
  // aynı vade tarihi için ise hayatı boyunca yalnızca bir bildirim üretilir.
