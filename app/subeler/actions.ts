...
export async function createBranch(formData: FormData) {
  ...
  const { data: lastBranch } = await supabase.from("branches").select("sort_order")
    .eq("organization_id", profile.organization_id).order("sort_order", { ascending: false }).limit(1).maybeSingle();
  ...
  const { error } = await supabase.from("branches").insert({
    ...
    is_active: true,
    sort_order: Number(lastBranch?.sort_order || 0) + 10
  });
...
export async function updateBranch(formData: FormData) {
  ...
  const { error } = await supabase.from("branches").update({
    name, pool_name: clean(formData.get("pool_name")) || null,
    address: clean(formData.get("address")) || null,
    location_url: clean(formData.get("location_url")) || null,
    contact_phone: clean(formData.get("contact_phone")) || null,
    whatsapp_phone: clean(formData.get("whatsapp_phone")) || null,
...
export async function reorderBranches(ids: string[]) {
  const profile = await requireProfile(["owner", "admin", "branch_manager"]);
  if (!profile.organization_id) throw new Error("Kurum bilgisi bulunamadı.");
  const supabase = await createClient();
  for (let i = 0; i < ids.length; i++) {
    const { error } = await supabase.from("branches").update({ sort_order: (i + 1) * 10, updated_at: new Date().toISOString() })
      .eq("id", ids[i]).eq("organization_id", profile.organization_id);
    if (error) throw error;
  }
  revalidateBranches();
}
