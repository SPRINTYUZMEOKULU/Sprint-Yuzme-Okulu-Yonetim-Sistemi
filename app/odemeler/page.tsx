...
export default async function PaymentsPage({
  ...
  const paymentRecords: PaymentRecord[] =
    payments.map((payment) => ({
      ...
      cancelled_by: payment.cancelled_by || null,
      cancelled_at: payment.cancelled_at || null,
      created_at: payment.created_at || null,
      due_date: payment.due_date || null,
    }));

  const validPayments = paymentRecords.filter(
    (payment) =>
      !payment.cancelled_at &&
      normalizeText(payment.payment_status) !== "cancelled"
  );
  ...
  const preparedStudents: PaymentStudent[] = students.map((student) => {
    ...
    const packagePrice = getPackagePrice(coursePackage);

    const enrollmentPayments = enrollment?.id
    ...
    const remainingPayment = Math.max(packagePrice - totalPaid, 0);
    ...
    const contactPhone = adult
      ? student.phone || student.guardian_phone || null
      : student.guardian_phone || student.phone || null;

    const latestPayment = enrollmentPayments[0] || null;
    ...
    return {
      ...
      due_date: dueDate,
      last_payment_at: latestPayment?.received_at || null,
    ...
    <main className="operationPage">
      <header className="operationHeader">
        <div>
          ...
          <h1>Ödeme Merkezi</h1>
          <span>
            Öğrenci ödemelerini, aktif paket tahsilatlarını, kayıt yenilemelerini,
            WhatsApp bildirimlerini ve kasa teslim süreçlerini tek merkezden yönetin.
          </span>
        </div>
      </header>
