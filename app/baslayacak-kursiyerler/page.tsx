...
export default async function StartingStudentsPage() {
  ...
  const { data: students } = await supabase
    .from("students")
    .select("id,student_number,first_name,last_name,phone,guardian_name,guardian_phone,branch_id,preferred_group_id,created_at")
  ...
  const [{ data: attendance }, { data: memberships }, { data: branches }, { data: groups }] = await Promise.all([
    ...
    ids.length
      ? supabase.from("student_group_memberships").select("student_id,group_id,started_at").in("student_id", ids).eq("is_active", true).order("started_at", { ascending: false })
  ...
  for (const m of (memberships || []) as MembershipRow[]) {
    if (!membershipMap.has(String(m.student_id))) membershipMap.set(String(m.student_id), m);
  }

  const branchMap = new Map<string, string | null>(
    ((branches || []) as BranchRow[]).map((b) => [String(b.id), b.name])
  );
  const groupMap = new Map<string, GroupRow>(
    ((groups || []) as GroupRow[]).map((g) => [String(g.id), g])
  );
    ...
    <main className="operationPage">
      <header className="operationHeader">
        <div>
          ...
          <span>Kesin kaydı tamamlanmış, aktif durumda olan ve henüz ilk yoklaması oluşmamış kursiyerleri takip edin.</span>
        </div>
      ...
      <section className="operationCard">
        <div className="sectionHead">
          <div>
            <p>İLK DERS TAKİBİ</p>
            <h2>{starting.length} kursiyer başlayacak</h2>
            <span>İlk yoklama kaydı oluştuğunda kursiyer bu listeden otomatik çıkar.</span>
          </div>
        </div>
        ...
        <div className="studentCardGrid">
          {starting.length ? (
            starting.map((s: any) => {
              const membership = membershipMap.get(String(s.id));
              const group = groupMap.get(String(membership?.group_id || s.preferred_group_id || ""));
              ...
              return (
                <article className="studentCard" key={s.id}>
                  <div>
                    ...
                    <p>{branch || "Şube belirtilmemiş"} · {group?.name || "Grup belirtilmemiş"}</p>
                    <p>Veli: {s.guardian_name || "—"} · Tel: {s.guardian_phone || s.phone || "—"}</p>
                  </div>
            ...
            <div className="tableEmpty">Şu anda ilk dersini bekleyen kursiyer bulunmuyor.</div>
          )}
        </div>
      </section>
