...
const menuItems: MenuItem[] = [
  ...
  { label: "Değişiklik ve Denetim", href: "/denetim-merkezi", moduleKey: "permissions" },
  ...
  return (
    <>
      <header className="proTopNav">
        <div className="proTopNavInner">
          ...
          <nav className="desktopLinks" aria-label="Ana menü">
            {loading ? (
              <span className="navLoading">Menü hazırlanıyor…</span>
            ) : (
          ...
          <div className="navActions">
            ...
            <button type="button" className="logout" onClick={logout} disabled={loggingOut}>
              <b>↪</b>
              <span>{loggingOut ? "Çıkılıyor…" : "Güvenli Çıkış"}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <aside className={`mobileDrawer ${menuOpen ? "open" : ""}`}>
        <header>
          <div>
            <small>SPRİNT YÜZME OKULU</small>
        ...
        <button type="button" className="drawerLogout" onClick={logout} disabled={loggingOut}>
      ...
      <style jsx global>{`
        ...
        .navBrandText{display:grid;line-height:1.08;min-width:130px}.navBrandText strong{font-size:16px;font-weight:950}.navBrandText small{margin-top:4px;color:#738198;font-size:10px}
        .desktopLinks{flex:1 1 auto;min-width:0;display:flex;align-items:center;justify-content:center;gap:3px;overflow-x:auto;scrollbar-width:none}.desktopLinks::-webkit-scrollbar{display:none}
        .desktopLinks a{flex:0 0 auto;min-height:38px;padding:0 10px;display:inline-flex;align-items:center;border-radius:10px;color:#5c697d;font-size:10.5px;font-weight:850;text-decoration:none;white-space:nowrap}.desktopLinks a:hover{color:#1769e8;background:#f1f7ff}.desktopLinks a.active{color:#fff;background:#1769e8;box-shadow:0 7px 17px rgba(23,105,232,.22)}
        .navLoading{color:#8492a6;font-size:11px}.navActions{flex:0 0 auto;display:flex;align-items:center;gap:7px}
        .hamburger{display:none;width:43px;height:42px;padding:0;border:1px solid #dbe4ef;border-radius:12px;background:#fff;color:#13233f;cursor:pointer;align-items:center;justify-content:center;flex-direction:column;gap:4px}.hamburger span{width:18px;height:2px;border-radius:8px;background:currentColor;transition:.18s ease}.hamburger.open span:nth-child(1){transform:translateY(6px) rotate(45deg)}.hamburger.open span:nth-child(2){opacity:0}.hamburger.open span:nth-child(3){transform:translateY(-6px) rotate(-45deg)}
        .logout{height:42px;padding:0 13px;display:flex;align-items:center;gap:7px;border:1px solid #fecdd3;border-radius:12px;background:#fff1f2;color:#be123c;font:900 11px system-ui;cursor:pointer}.logout b{font-size:19px}.logout:disabled,.drawerLogout:disabled{opacity:.6;cursor:wait}
        ...
        .mobileDrawer{position:fixed;z-index:1100;top:0;right:0;width:min(88vw,380px);height:100dvh;padding:18px;background:#071b3b;color:#fff;transform:translateX(105%);transition:transform .22s ease;box-shadow:-24px 0 60px rgba(3,12,28,.38);overflow-y:auto}.mobileDrawer.open{transform:translateX(0)}
        .mobileDrawer header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding-bottom:15px;border-bottom:1px solid rgba(255,255,255,.13)}.mobileDrawer header div{display:grid;gap:4px}.mobileDrawer header small{color:#82b8ff;font-size:10px;font-weight:900;letter-spacing:1px}.mobileDrawer header strong{font-size:21px}.mobileDrawer header button{width:42px;height:42px;border:1px solid rgba(255,255,255,.2);border-radius:13px;background:rgba(255,255,255,.09);color:#fff;font-size:27px;cursor:pointer}
        .mobileDrawer nav{display:grid;gap:7px;padding:16px 0}.mobileDrawer nav a{min-height:50px;padding:0 15px;display:flex;align-items:center;justify-content:space-between;border-radius:13px;color:#c9d6e8;background:rgba(255,255,255,.04);text-decoration:none;font:800 14px system-ui}.mobileDrawer nav a.active{color:#fff;background:#1769e8;box-shadow:0 8px 22px rgba(23,105,232,.25)}
        ...
        /* Mobilde tek navigasyon kaynağı app/layout.tsx içindeki GlobalMobileNav'dır.
           UstGezinme'nin eski mobil bar/drawer'ı tamamen kapatılır; böylece çift menü,
           sağdan açılan eski panel ve altta görünen geri oku ortadan kalkar. */
        @media(max-width:820px){
          .proTopNav,.menuShade,.mobileDrawer{display:none!important}
          html.topMenuOpen{overflow:auto!important}
        }
      `}</style>
    </>
  );
