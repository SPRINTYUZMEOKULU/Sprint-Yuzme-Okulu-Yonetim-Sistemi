...
export default function SidebarToggle() {
  ...
  useEffect(() => {
    const links = Array.from(
      document.querySelectorAll<HTMLAnchorElement>(".proSidebar .proNavItem"),
    );
    ...
    links.forEach((link) => {
      ...
      childBox.innerHTML = children
        .map(
          (child) =>
            `<a href="${child.href}" class="sidebarBranchChild"><i></i><span>${child.label}</span><b>→</b></a>`,
        )
        .join("");
  ...
  return (
    <>
      <button
        ...
        aria-label={collapsed ? "Sol menüyü aç" : "Sol menüyü daralt"}
      ...
      <style jsx global>{`
        ...
        .sprintSidebarToggle:hover{transform:translateY(-1px);border-color:#b9d3f8;background:#f3f8ff}
        ...
        .proSidebar .proNavItem.branchOpen .sidebarBranchToggle{transform:translateY(-50%) rotate(180deg);color:#dbeafe}
        .proSidebar .sidebarBranchChildren{display:grid;gap:3px;margin:2px 4px 7px 42px;padding:4px 0 2px 10px;border-left:1px solid rgba(122,161,211,.22);animation:sprintBranchOpen .16s ease}
        .proSidebar .sidebarBranchChildren[hidden]{display:none!important}
        .proSidebar .sidebarBranchChild{min-height:34px;padding:6px 8px;display:grid;grid-template-columns:7px minmax(0,1fr) 15px;align-items:center;gap:7px;border-radius:9px;color:#8fa4c2;text-decoration:none;font-size:11.5px;font-weight:760;line-height:1.2}
        .proSidebar .sidebarBranchChild:hover{background:rgba(44,124,231,.12);color:#e3efff}
        ...
        html.sprintSidebarCollapsed .proBrand>div:last-child{display:none}
        html.sprintSidebarCollapsed .navGroup>p{display:none}
        html.sprintSidebarCollapsed .proNavItem{justify-content:center;padding-left:0;padding-right:0}
        html.sprintSidebarCollapsed .proNavItem>span,html.sprintSidebarCollapsed .proNavItem>b{display:none}
        html.sprintSidebarCollapsed .sidebarBranchToggle,html.sprintSidebarCollapsed .sidebarBranchChildren{display:none!important}
        ...
        @media(max-width:820px){
          ...
          html.sprintSidebarMobileOpen .proSidebar{transform:translateX(0)}
          html.sprintSidebarMobileOpen{overflow:hidden}
          html.sprintSidebarMobileOpen .sprintSidebarToggle{visibility:hidden;pointer-events:none}
          .sprintMobileMenuOverlay{display:block;position:fixed;z-index:7000;inset:0;width:100%;height:100%;margin:0;padding:0;border:0;background:rgba(4,14,32,.56);cursor:pointer}
          .sprintMobileMenuClose{display:inline-flex;position:fixed;z-index:99999;top:calc(env(safe-area-inset-top,0px) + 14px);left:calc(min(86vw,310px) - 58px);width:44px;height:44px;align-items:center;justify-content:center;padding:0;border:2px solid rgba(255,255,255,.75);border-radius:14px;background:#fff;color:#10213a;cursor:pointer;box-shadow:0 10px 30px rgba(3,15,36,.3)}
          .sprintMobileMenuClose span{position:absolute;width:21px;height:2.5px;border-radius:999px;background:currentColor}
          .sprintMobileMenuClose span:first-child{transform:rotate(45deg)}
          .sprintMobileMenuClose span:last-child{transform:rotate(-45deg)}
          html .proNav{display:block;overflow:visible;padding-top:18px}
          html .navGroup{display:block;margin-bottom:18px}
          html .navGroup>p{display:block}
          html .proNavItem{justify-content:flex-start;min-width:0;padding:0 11px}
          html .proNavItem.hasBranchMenu{padding-right:46px}
          .proSidebar .sidebarBranchToggle{right:5px;width:38px;height:40px}
          .proSidebar .sidebarBranchChildren{margin-left:40px;padding-left:10px}
          .proSidebar .sidebarBranchChild{min-height:38px;font-size:12px}
          html .proUser{display:grid}
          .proTopbar{gap:10px}
          .proTopbar>div:first-of-type{min-width:0;flex:1}
        }
      `}</style>
    </>
  );
