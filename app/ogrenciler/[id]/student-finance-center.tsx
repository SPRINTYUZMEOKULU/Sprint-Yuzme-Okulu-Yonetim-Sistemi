...
export default function StudentFinanceCenter({ studentId }: { studentId: string }) {
  ...
  const paymentDueTime = data?.enrollment?.paymentDueDate
    ? new Date(`${data.enrollment.paymentDueDate.slice(0, 10)}T23:59:59+03:00`).getTime()
    : Number.NaN;
        ...
        <div className="sfcBody">
          ...
          {tab === "history" ? (
            <section className="sfcCard">
              ...
              {data && (data.payments.length > 0 || packageRemaining > 0) ? (
                <div className="sfcHistory">
                  {data.enrollment && packageRemaining > 0 ? (
                    <article className={`sfcUnpaidPeriod${packagePaymentOverdue ? " overdue" : ""}`}>
                      <div>
                        <b>{money(packageRemaining)} · {packagePaymentOverdue ? "VADESİ GEÇTİ" : "ÖDEME BEKLİYOR"}</b>
                        <span>{data.enrollment.lessonCount > 0 ? `${data.enrollment.lessonCount} derslik ` : ""}{data.enrollment.packageName || "Aktif kurs paketi"}</span>
                        <span>{dateText(data.enrollment.startDate)} - {dateText(data.enrollment.plannedEndDate)} tarihleri arasındaki paket ücreti ödenmedi.</span>
