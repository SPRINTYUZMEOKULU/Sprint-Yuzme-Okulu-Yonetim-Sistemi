...
export type CorrectionRow = {
  ...
  normal_end_date: string;
  compensation_end_date: string;
  package_lesson_count: number;
  total_lessons: number;
  used_lessons: number;
  normal_remaining: number;
  compensation_remaining: number;
...
const columns: Array<[keyof CorrectionRow, string, boolean]> = [
  ["student_id", "__STUDENT_ID", false],
  ["student_number", "Öğrenci No", false],
  ["student_name", "Öğrenci Ad Soyad", false],
...
function parseCSV(text: string) {
  ...
  row.push(cell.replace(/\r$/, "")); if (row.some((x) => x !== "")) result.push(row);
  return result;
}
...
export default function DataCorrectionClient({ rows, branches, groups, packages }: Props) {
  ...
  const issueCount = rows.filter((r) => r.issues).length;
  const paymentIssue = rows.filter((r) => r.payment_outstanding > 0).length;
  ...
  function exportFile(all = false) {
    const exportRows = all ? rows : visible;
  ...
  async function readImport(file?: File) {
    if (!file) return;
    setMessage("Dosya kontrol ediliyor…"); setPreview([]);
    try {
      const matrix = parseCSV(await file.text());
      if (matrix.length < 2) throw new Error("Dosyada veri satırı bulunamadı.");
      const headers = matrix[0].map(normalized);
      const idIndex = headers.indexOf("__STUDENT_ID");
      const noIndex = headers.indexOf("Öğrenci No");
      ...
      const currentMap = new Map(rows.map((r) => [r.student_id, r]));
      const next: PreviewRow[] = [];
      for (const cells of matrix.slice(1)) {
        const id = normalized(cells[idIndex]); if (!id) continue;
        const current = currentMap.get(id);
        const studentNo = normalized(cells[noIndex]);
        ...
        headers.forEach((header, index) => {
          const key = headerToKey.get(header); if (!key || !editableKeys.has(key)) return;
          const incoming = normalized(cells[index]);
          if (!incoming) return; // boş hücre mevcut doğru veriyi silmez
          const currentValue = normalized(current[key]);
          const compareIncoming = incoming === "TEMİZLE" ? "" : incoming;
          if (compareIncoming !== currentValue) changes[String(key)] = incoming;
        });
      ...
      setPreview(next);
      const errors = next.filter((x) => x.error).length;
      const changes = next.filter((x) => !x.error).length;
      setMessage(`${matrix.length - 1} satır okundu · ${changes} öğrenci güncellenecek${errors ? ` · ${errors} hatalı satır` : ""}.`);
    } catch (e) { setMessage(e instanceof Error ? e.message : "Dosya okunamadı."); }
    finally { if (inputRef.current) inputRef.current.value = ""; }
  }

  async function applyImport() {
    const valid = preview.filter((x) => !x.error && Object.keys(x.changes).length);
    ...
    try {
      const response = await fetch("/api/student-data-correction-import", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ rows: valid }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Güncelleme tamamlanamadı.");
      setMessage(`${result.updated || 0} öğrenci güncellendi.${result.errors?.length ? ` ${result.errors.length} satır uygulanamadı.` : ""}`);
      setPreview([]); router.refresh();
    } catch(e) { setMessage(e instanceof Error ? e.message : "Güncelleme sırasında hata oluştu."); }
    finally { setApplying(false); }
  }

  return <section className="dataCorrectionShell">
    <div className="qualityCards">
      <button onClick={()=>setOnlyIssues(false)}><span>Toplam Kayıt</span><strong>{rows.length}</strong></button>
      <button onClick={()=>setOnlyIssues(true)}><span>Eksik / Kontrol</span><strong>{issueCount}</strong></button>
      <button onClick={()=>{setOnlyIssues(false);setQuery("Telefon eksik")}}><span>Telefon Eksik</span><strong>{contactIssue}</strong></button>
      <button onClick={()=>{setOnlyIssues(false);setQuery("Ödeme bekliyor")}}><span>Ödeme Bekleyen</span><strong>{paymentIssue}</strong></button>
    </div>

    <div className="correctionToolbar">
      <div><h2>Toplu Veri Temizleme</h2><p>Excel’de öğrenciyi her zaman <b>Ad Soyad + Öğrenci No</b> ile görürsünüz. <code>__STUDENT_ID</code> eşleştirme için korunur.</p></div>
      <div className="toolbarActions">
        <button className="primary" onClick={()=>exportFile(false)}>Eksik / Filtreli Veriyi Excel’e Aktar</button>
        <button onClick={()=>exportFile(true)}>Tüm Detayları Excel’e Aktar</button>
        <button onClick={()=>inputRef.current?.click()}>Düzeltilmiş Excel/CSV’yi Kontrol Et</button>
        <input ref={inputRef} type="file" accept=".csv,text/csv" hidden onChange={(e)=>void readImport(e.target.files?.[0])}/>
      </div>
    </div>

    <div className="referenceNote"><b>Güvenli içe aktarma:</b> Boş hücreler mevcut veriyi silmez. Bir alanı özellikle temizlemek için hücreye <b>TEMİZLE</b> yazın. Şube, Grup ve Paket değerlerini sistemdeki adlarıyla kullanın.<details><summary>Geçerli referansları göster</summary><p><b>Şubeler:</b> {branches.join(" · ")}</p><p><b>Paketler:</b> {packages.join(" · ")}</p><p><b>Gruplar:</b> {groups.join(" · ")}</p></details></div>
