      ...
      <form action={updateGroupMulti} className="groupEditorForm">
        ...
        <div className="groupEditorBody">
          ...
          <div className="editorGrid">
            ...
            <fieldset className="levelField"><legend>Seviyeler <small>Birden fazla seçebilirsiniz</small></legend><div className="levelGrid">
              {levels.map((level) => <label key={level.id} className={level.id === group.level_id ? "currentLevel" : ""}><input type="checkbox" name="level_ids" value={level.id} defaultChecked={level.id === group.level_id} disabled={level.id === group.level_id} /><span><b>{level.name}</b>{level.id === group.level_id && <em>Mevcut seviye</em>}</span>{level.id === group.level_id && <input type="hidden" name="level_ids" value={level.id} />}</label>)}
            </div><p className="levelHelp">Birden fazla seviye seçildiğinde aynı şube, gün ve saatte her seviye için ayrı grup oluşturulur. Öğrenci, kontenjan ve yoklama kayıtları ayrı takip edilir.</p></fieldset>
