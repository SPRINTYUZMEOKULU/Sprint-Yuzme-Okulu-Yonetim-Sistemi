...
export default async function AttendancePage({
  ...
  if (!organizationId) return <main style={{minHeight:"100vh",padding:32,background:"#f4f7fb",color:"#10213a"}}><h1>Yoklama</h1><p>Organizasyon bilgisi bulunamadı.</p><Link href="/">← Yönetim Paneline Dön</Link></main>;
  ...
  if(error)return <main style={{minHeight:"100vh",padding:32,background:"#f4f7fb",color:"#10213a"}}><h1>Yoklama</h1><div style={{marginTop:18,padding:18,background:"#fff",border:"1px solid #fecaca",borderRadius:14,color:"#991b1b"}}>Veriler yüklenemedi: {error.message}</div></main>;
  ...
  return <>
    <nav className="saTopNav" aria-label="Yoklama hızlı erişim">
      ...
      <Link href="/yoklama/aylik" className="saTopNavItem"><span className="saTopNavIcon">▦</span><span><b>Tüm Ayı Gör</b><small>Yoklama Takvimi</small></span></Link>
      <Link href="/yoklama/gecmis" className="saTopNavItem"><span className="saTopNavIcon">↶</span><span><b>Geçmiş</b><small>Yoklama Kayıtları</small></span></Link>
    </nav>
    <SessionAttendanceClient branches={branches.data||[]} groups={groups.data||[]} schedules={schedules.data||[]} memberships={memberships.data||[]} students={students.data||[]} enrollments={enrollments.data||[]} compensationLessons={compensation.data||[]} profiles={profiles.data||[]} levels={levels.data||[]} attendanceHistory={attendanceHistory.data||[]} staffAssignments={staffAssignments.data||[]} initialBranchId={params.branchId||""}/>
  </>;
}
