...
export default function AttendanceClient({
  ...
  const trackableStudents = useMemo(
    () =>
      attendanceStudents.filter(
        (student) => !normalTrackingEndedIds.has(student.id)
      ),
    [attendanceStudents, normalTrackingEndedIds]
  );
          ...
          <section style={panelStyle}>
            ...
            {filteredStudents.map(
              (student) => {
                ...
                const trackingEnded =
                  normalTrackingEndedIds.has(student.id);
                    ...
                    >
                      ...
                      >
                        ...
                        {trackingEnded && (
                          ...
                          >
                            🔴 DERS HAKKI BİTMİŞTİR · DERS TAKİBİ TAMAMLANMIŞTIR
                            <div style={{ marginTop: 3, fontWeight: 700 }}>
                              Normal yoklama kapalıdır. Kayıt yenileme işlemi yapılmalıdır. Aktif telafi dersi varsa telafi yoklaması ayrıca yapılabilir.
                            </div>
                          </div>
                        )}
                      </div>
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (trackingEnded) {
                                    setMessage(
                                      `${student.first_name || "Öğrencinin"} ders hakkı bitmiştir. Ders takibi tamamlanmıştır; kayıt yenileme yapılmalıdır.`
                                    );
                                    return;
                                  }
                                ...
                                style={{
                                  ...
                                  lineHeight: 1.2,
                                  whiteSpace: "normal",
                                  cursor: trackingEnded ? "not-allowed" : "pointer",
                                  opacity: trackingEnded ? 0.45 : 1,
                                }}
                              >
                                {isCompensationLesson &&
                                status === "compensation"
