...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <main className="ahRoot">
  ...
  <form className="ahFilter ahHistoryFilter"><label>Şube<select name="branchId" defaultValue={branchId}><option value="">Tüm Şubeler</option>{(branches||[]).map((b:any)=><option key={b.id} value={b.id}>{b.short_name||b.name}</option>)}</select></label><label>Grup<select name="groupId" defaultValue={groupId}><option value="">Tüm Gruplar</option>{branchGroups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select></label><label>Saat<select name="time" defaultValue={time}><option value="">Tüm Saatler</option>{availableTimes.map(item=><option key={item} value={item}>{item}</option>)}</select></label><label>Durum<select name="status" defaultValue={status}><option value="">Tüm Durumlar</option><option value="present">Katıldı</option><option value="absent">Katılmadı</option><option value="excused">İzinli</option><option value="compensation">Telafi</option></select></label><label>Sıralama<select name="sort" defaultValue={sort}><option value="newest">En yeni → eski</option><option value="oldest">En eski → yeni</option><option value="name">İsim A → Z</option><option value="name-desc">İsim Z → A</option></select></label><label>Başlangıç<input type="date" name="from" defaultValue={from}/></label><label>Bitiş<input type="date" name="to" defaultValue={to}/></label><button>Filtrele</button><Link href="/yoklama/gecmis">Temizle</Link></form>
