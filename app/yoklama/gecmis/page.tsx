...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const exportRows=filtered.map((r:any)=>({date:tr(r.lesson_date),time:timeLabel(scm.get(r.schedule_id)?.start_time),student:sm.get(r.student_id)?.name||"Kursiyer",branch:bm.get(r.branch_id)||"Şube",group:gm.get(r.group_id)?.name||"Grup",status:statusLabel(r.status),note:r.coach_note||"",renewal:""}));
 ...
 return <main className="ahRoot">
  ...
  <div className="ahDays">{[...dates.entries()].map(([date,list])=><section className="ahDay" key={date}><header><strong>{tr(date)}</strong><div className="ahDayHeadActions"><span>{list.length} kayıt</span><Link href={`/yoklama?date=${date}`} className="ahDayButton">Bu Güne Git</Link></div></header>{list.map((r:any,i:number)=>{const schedule=scm.get(r.schedule_id);const group=gm.get(r.group_id);return <div className="ahRow ahHistoryRow" key={`${r.id||r.student_id}-${i}`}><div className="ahRowMain"><b>{sm.get(r.student_id)?.name||"Kursiyer"}</b><small>{bm.get(r.branch_id)||"Şube"} · {group?.name||"Grup"}{timeLabel(schedule?.start_time)?` · ${timeLabel(schedule.start_time)}${schedule?.end_time?`–${timeLabel(schedule.end_time)}`:""}`:""}</small>{r.coach_note&&<span className="ahNoteFlag">Not: {r.coach_note}</span>}<div className="ahInlineActions"><Link href={`/ogrenciler/${r.student_id}`}>Öğrenci Dosyası</Link><Link href={`/yoklama?date=${r.lesson_date}&branchId=${r.branch_id||""}`}>Yoklamayı Aç</Link></div></div><span className={`st ${r.status}`}>{r.status==="present"?"✓ Katıldı":r.status==="absent"?"✕ Katılmadı":r.status==="excused"?"○ İzinli":"T Telafi"}</span></div>})}</section>)}{!dates.size&&<div className="ahEmpty">Bu filtrelere uygun geçmiş yoklama kaydı bulunamadı.</div>}</div>
 </main>
}
